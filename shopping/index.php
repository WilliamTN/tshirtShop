...
/*
	Author: TN William from template by Faisal Bin Ghimlas
...
try
{
	$result = $pdo->query('SELECT shipping_region FROM shipping_region');
}
	 ...
	 $validation_errors="";
	    ...
	    if($validator->ValidateForm())
	    {
	        	...
	        	try
				{
					...
					$s->bindValue(':order_net_value', $_POST['deliveryRate'] + get_order_total($link));
					...
					if(isset($_SESSION['cart']) && is_array($_SESSION['cart'])){
						$max=count($_SESSION['cart']);
		   				...
		   				$sql = 'INSERT INTO ordereditem SET
								...
								$s->bindValue(':selling_price', get_price($_SESSION['cart'][$i]['productid'],$link));
					...
					if(isset($_SESSION['cart']) && is_array($_SESSION['cart'])){
						$max=count($_SESSION['cart']);
		   				...
		   				$sql = 'INSERT INTO order_detail SET
								...
								$s->bindValue(':product_id', $_SESSION['cart'][$i]['product_id']);
								$s->bindValue(':quantity_ordered', $_SESSION['cart'][$i]['qty']);
								$s->bindValue(':selling_price', get_price($_SESSION['cart'][$i]['product_id'],$link));
					...
					unset($_SESSION['cart']);
		   			...
		   			header('Location: ../myaccount/index.php?orderhistory');
		   			exit();
				}
				catch (PDOException $e)
				{
...
else if(isset($_GET['checkout']))
{
	...
	if(!isset($_SESSION['loggedin'])) {
		header('Location: ../myaccount/loginform.html.php');
		exit();
	}
...
else if (isset($_GET['calculatedelivery']))
{
	...
	unset($_SESSION['cart']);
	header('Location: /BookStore/bookstore/books');
	exit();
}
